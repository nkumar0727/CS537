...
{
	// NULL POINTER DEREFERENCE - check 0
	/** cprintf("String Address: %p\n", addr); */
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
	...
	// NULL POINTER DEREFERENCE - check if 0
  if((uint)i >= proc->sz || (uint)i+size > proc->sz || (uint)(i) == 0 || (uint)i < 0x1000)
    return -1;
  *pp = (char*)i;
  return 0;
}
